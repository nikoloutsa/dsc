...
#SBATCH --output=logs/horovod_tensorflow_synthetic_benchmark.gloo.N16.G2.B32.%j.out 
#SBATCH --error=logs/horovod_tensorflow_synthetic_benchmark.gloo.N16.G2.B32.%j.err 
#SBATCH --ntasks=32
#SBATCH --gres=gpu:2
#SBATCH --nodes=16
...
horovodrun --gloo -np $SLURM_NTASKS -H $WORKERS --network-interface ib0 --start-timeout 120 --gloo-timeout-seconds 120 \
    python -u train.horovod.tensorflow.synthetic.py --batch-size 32 --num-batches-per-iter ${1:-16} --num-iters 3
